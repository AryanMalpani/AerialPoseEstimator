...
= 1, dilation: int = 1, output_padding: int = 0) -> nn.Conv2d:
...
https://ngc.nvidia.com/catalog/model-scripts/nvidia:resnet_50_v1_5_for_pytorch.
    ...
    def __init__(
        ...
        # Both self.conv2 and self.upsample layers upsample the input when 
stride != 1
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.de_conv1 = nn.ConvTranspose2d(64, 3, kernel_size=7, stride=2, 
padding=3, bias=False)
        self.unpool = nn.MaxUnpool2d(kernel_size=3, stride=2, padding=1)
        self.bn1 = norm_layer(3)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.unsample = nn.Upsample(size=[45,30], mode='nearest')
...
ignore[arg-type]
                elif isinstance(m, BasicBlock) and m.bn2.weight is not 
None:
...
output_padding==0:
            upsample = nn.Sequential(
                conv1x1(planes * block.expansion, last_block_dim, stride, 
output_padding),
                # norm_layer(planes * block.expansion),
                norm_layer(last_block_dim),
            )
        last_block = block(
                last_block_dim, planes, stride, output_padding, upsample, 
...
if __name__ == "__main__":
    ...
    resnet50 = ResNet(Bottleneck, [1,1,1,1])
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    resnet50.to(device)
    test = torch.zeros([1,2048,1,1]).to(device)
    print(resnet50(test).shape)
...
# def resnet50(*, weights: Optional[ResNet50_Weights] = None, progress: 
bool = True, **kwargs: Any) -> ResNet:
#     return _resnet(Bottleneck, [3, 4, 6, 3], weights, progress, 
**kwargs)
 
 
# def resnet101(*, weights: Optional[ResNet101_Weights] = None, progress: 
bool = True, **kwargs: Any) -> ResNet:
#     return _resnet(Bottleneck, [3, 4, 23, 3], weights, progress, 
**kwargs)
